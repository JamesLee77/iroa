...
아래 값은 로고 마스터와 인쇄물의 기준이다. 웹 제품은 화면 매체에 맞춘 별도 팔레트를 사용하며 3.4에서 그 위치를 밝힌다.
...
웹 제품은 위 브랜드 색을 그대로 칠하지 않는다. 화면은 인쇄와 요구가 달라 잉크를 순수 검정 대신 브랜드 네이비를 푼 값으로 두고, 행동색은 밝은 배경에서 흰 라벨을 지탱할 만큼 깊게 내린다.

그 값의 정본은 이 문서가 아니라 `src/styles/tokens.css`의 시맨틱 토큰이다. 대비 측정값은 `src/lib/design-system/color-roles.ts`가 토큰에서 직접 계산해 사이트의 디자인 시스템 페이지에 게시하며, 자동 검사가 3.2의 기준 아래로 내려가지 않도록 고정한다. 화면 값을 이 문서에 옮겨 적지 않는 이유는 진실을 두 곳에 두지 않기 위해서다.

로고 마스터는 3.1의 브랜드 색을 유지한다. 워드마크의 코랄 행동점은 마크의 고유 장치이며 화면 팔레트의 행동색과 별개다.

## 4. 서체와 문장

### 4.1 서체

- 화면(한국어·영문 공통): **Pretendard Variable**, 유니코드 범위별 서브셋으로 적재
- 화면 대체: **Apple SD Gothic Neo**, **Malgun Gothic**, 시스템 sans-serif
- 화면의 수치·식별자: 시스템 고정폭 서체 — 값이 값으로 읽히도록 본문과 구분한다
- 인쇄·문서: **Noto Sans KR** Medium / Bold, 영문과 숫자는 **Noto Sans** 또는 **Arial**
